...
\begin{image}
\includegraphics[width=5cm]{xmPictures/PFR_met_recycle.png}
\end{image} 
...
Bij monotoon stijgende kinetiek geldt:

\begin{itemize}
\item Meer recycle verhoogt het totale debiet door de reactor.
\item De concentratiegradiënt in de reactor wordt kleiner.
\item Het benodigde volume om dezelfde conversie te behalen neemt toe.
\item In de limiet $R \rightarrow \infty$ wordt het vereiste volume gelijk aan dat van een CSTR.
\end{itemize}


Voor een isotherm proces met monotoon stijgende kinetiek
(en constante dichtheid) geldt dat de reactiesnelheid
afneemt met dalende concentratie. In dat geval is een ideale PFR ($R=0$) het meest volume-efficiënt.

Het toevoegen van recycle verhoogt het totale debiet
door de reactor met een factor $(R+1)$ en verlaagt
de inlaatconcentratie. Hierdoor wordt de gemiddelde
reactiesnelheid kleiner en neemt het benodigde volume om dezelfde conversie te behalen toe. In de limiet $R \rightarrow \infty$ wordt het vereiste volume gelijk aan dat van een CSTR.
