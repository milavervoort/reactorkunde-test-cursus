...
\title{Oefening: Autokatalytische reactie}
...
In de theorie werden de ontwerpvergelijkingen voorgesteld en bekeken voor reacties met een \textbf{monotoon stijgende kinetiek}. Voor dergelijke reacties neemt de reactiesnelheid af naarmate de concentratie van het reagens afneemt. Hieruit volgde bijvoorbeeld dat een PFR voor eenzelfde conversie steeds een kleiner reactorvolume vereist dan een CSTR.

Niet alle reacties volgen echter een monotoon stijgende kinetiek. Bij sommige reacties, zoals een \textbf{autokatalytische reactie}, neemt de reactiesnelheid aanvankelijk toe doordat één van de reactieproducten zelf als katalysator optreedt. De grafiek van $1/(-R_A)$ als functie van de concentratie krijgt hierdoor een andere vorm.

In deze oefening onderzoeken we hoe de kinetiek de keuze van het reactortype beïnvloedt. 

\begin{definition} \textbf{Autokatalytische reactie}
    
Een autokatalytische reactie is een chemische reactie waarbij één van de producten de reactie versnelt door als katalysator te fungeren. Met andere woorden, de reactie “katalyseert zichzelf”.  
Een typisch voorbeeld van een autokatalytische reactie is:

\[A \rightarrow B\]

Hierbij is \(B\) zowel een product als een katalysator voor de reactie. De aanwezigheid van product B versnelt de reactie. De reactiesnelheid neemt daardoor aanvankelijk toe, bereikt een maximum en neemt vervolgens opnieuw af doordat de concentratie van A steeds kleiner wordt.
...
Bij constante densiteit blijft de totale molaire concentratie in de reactor constant. Er geldt dus: $C_{tot} = C_{A0} + C_{B0} = C_A + C_B$.
Hieruit volgt dat de concentratie van component $B$ kan worden uitgedrukt als:
\[
C_B = C_{B0} + C_{A0} - C_A
\]
...
Wanneer $\frac{1}{-R_A}$ wordt uitgezet tegen $C_A$, ziet de curve er als volgt uit:
...
De kromme van $1/(-R_A)$ als functie van $C_A$ verschilt duidelijk van deze bij een monotoon stijgende kinetiek. De reactiesnelheid bereikt een maximum bij een intermediaire waarde van de concentratie (of conversie). 
Hierdoor verandert ook de relatieve performantie van de verschillende reactortypes.

Bij een isotherme reactie met monotoon stijgende kinetiek geldt:
\[
\tau_{\text{CSTR}}>\tau_{\text{PFR}}
\]
Bij een autokatalytische reactie is dit echter niet langer gegarandeerd.

Omdat de reactiesnelheid bij lage conversie zeer klein is (weinig product aanwezig), werkt een PFR in het begin van zijn lengte in een
ongunstig kinetisch gebied. Een CSTR daarentegen opereert volledig aan de uitlaatconcentratie. Indien deze zich dichter bij het snelheidsmaximum bevindt dan de lage inlaatconcentratie van een PFR, kan de CSTR bij lage conversies een kleinere verblijftijd vereisen dan de PFR.\\
...
\begin{itemize}
\item Bij lage conversie ($X_A$ < 0.5) is de CSTR performanter dan een PFR.
\item Bij hogere conversie ($X_A$ > 0.5) wordt de PFR opnieuw voordeliger.
\end{itemize}

Ook andere reactorconfiguraties kunnen performanter zijn.
\end{conclusion}
\vspace{1cm}
In de volgende oefening wordt deze invloed van de kinetiek op het reactorontwerp kwantitatief onderzocht. Verschillende reactorconfiguraties worden met elkaar vergeleken en geoptimaliseerd.
...
\begin{hint} 
    ...
    Bereken vervolgens het benodigde volume van de PFR om van $C_A^*$ naar $C_{A,uit}$ te gaan.
\end{hint}
