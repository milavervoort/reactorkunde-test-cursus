...
Een \textbf{neuraal netwerk} is een Machine Learning-model dat een verband
tussen invoer- en uitvoergegevens kan leren. Het netwerk bestaat uit
verschillende lagen waarin eenvoudige berekeningen worden uitgevoerd.
...
In tegenstelling tot een klassieke kinetische vergelijking leggen we de
vorm van $f_\theta$ niet vooraf vast. Het netwerk leert deze relatie uit de
beschikbare gegevens.


\begin{remark} \textbf{Neuraal netwerk als black-boxmodel}

Bij een klassieke kinetische vergelijking kiezen we vooraf een fysische vorm,
bijvoorbeeld
...
Bij een neuraal netwerk wordt de functionele vorm daarentegen niet vooraf
opgelegd. Het netwerk leert zelf een geschikte relatie tussen de invoer en
uitvoer.

Een neuraal netwerk wordt daarom vaak een \textbf{black-boxmodel} genoemd:
de invoer en uitvoer zijn bekend, maar de volledige geleerde relatie is
doorgaans moeilijk fysisch te interpreteren.
\end{remark}
...
De inputlaag bevat de variabelen die aan het netwerk worden aangeboden.
De hidden layers voeren berekeningen uit en de outputlaag produceert de
uiteindelijke voorspelling.

De structuur van een neuraal netwerk bepaalt dus hoe de informatie door het model stroomt. De exacte relatie die het netwerk leert, wordt echter bepaald door de parameters van de neuronen in elke laag. 
...
De eerste twee stappen zijn eenvoudig: het netwerk maakt een voorspelling via forward propagation en deze voorspelling wordt vergeleken met de gewenste uitvoer via de loss functie. Het bijzondere aan neurale netwerken is dat er vaak zeer veel trainbare parameters zijn.
We moeten daarom bepalen hoe de loss verandert wanneer elk van deze
parameters wordt aangepast.
...
bepalen. Deze geeft aan hoe de loss verandert wanneer $\theta$ wordt
aangepast.
...
\]
Bij een neuraal netwerk wordt dit principe toegepast op alle weights en biases. De berekening begint daarbij bij de loss en wordt vervolgens
achterwaarts door het netwerk uitgevoerd. Vandaar de naam
\textbf{backpropagation}.
\end{definition}



De berekende gradiënten worden vervolgens door een optimizer gebruikt om de parameters aan te passen. Conceptueel gebeurt bijvoorbeeld bij gradient descent:

\[\theta
=
\theta-\eta\nabla_\theta L
\]
waarbij $\eta$ de learning rate is.

In de praktijk kan hiervoor bijvoorbeeld de Adam-optimizer worden gebruikt. De precieze werking van de optimizer is voor het begrijpen van neurale netwerken niet essentieel; belangrijk is dat de optimizer de met backpropagation berekende gradiënten gebruikt om de weights en biases aan te passen.
