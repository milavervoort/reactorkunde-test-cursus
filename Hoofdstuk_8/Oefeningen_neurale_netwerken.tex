...
Voor dit probleem gebruiken we de \textit{Mean Squared Error (MSE)} als lossfunctie. In PyTorch kunnen we de MSE-loss aanmaken met:
\begin{verbatim}
...
mogen zijn. De learning rate is een belangrijke trainingsinstelling. 

Een te grote learning rate kan ervoor zorgen dat de training instabiel wordt, terwijl
...
In de Google Colab-notebook is de trainingsloop grotendeels voorgeprogrammeerd. Bekijk eerst de verschillende stappen in de code. Vul daarna de lege regels in de trainingsloop aan.

De volledige trainingsloop kan nu worden uitgevoerd. 
...
We willen nu voor elke $x$-waarde een voorspelling van het getrainde netwerk berekenen. 

Vul in Google Colab de ontbrekende code aan, zodat het netwerk voor alle waarden in \texttt{x\_plot} een voorspelling \texttt{y\_plot} genereert. \\
Maak vervolgens een grafiek waarin de oorspronkelijke trainingsdata wordt weergegeven en vergeleken met de voorspellingen van het neurale netwerk.
...
We kiezen bijvoorbeeld een waarde die ook binnen het bereik van onze trainingsdata ligt:
...
Vergelijk de voorspelling met de exacte waarde van $y=x^2+1.$

\[
y=7^2+1=50
\]

Stel dat het netwerk bijvoorbeeld een waarde van $49.2$ voorspelt. 

Welke conclusie is dan het meest correct?
